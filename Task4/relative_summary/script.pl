...
foreach my $warn_type(sort {lc $a cmp lc $b} keys %warnings_af) {
...
foreach my $warn_type(sort {lc $a cmp lc $b} keys %warnings_bef) {
  my $bef = $warnings_bef{$warn_type};
  my $af = $warnings_af{$warn_type};
  if(!$af) {
    $af=0;
...
$file_out_handle->print("\n\n", "ERROR_TYPE    BEFORE_PATCH   AFTER_PATCH");
foreach my $err_type(sort {lc $a cmp lc $b} keys %errors_af) {
